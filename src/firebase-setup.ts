...
import { ProjectConfig, FirebaseConfig } from './types.js';
import { logSuccess, logWarning, logInfo } from './utils.js';

export async function setupFirebase(config: ProjectConfig): Promise<void> {
...
async function checkFirebaseCLI(): Promise<void> {
  try {
    ...
    throw new Error(
      'Firebase CLI is not installed. Install it with: npm install -g firebase-tools'
    );
  }
}

async function createFirebaseRC(config: ProjectConfig): Promise<void> {
  const firebaseRC = {
    projects: {
...
async function updateFirebaseJson(config: ProjectConfig): Promise<void> {
...
async function createEnvFiles(config: ProjectConfig): Promise<void> {
...
function getRunCommand(config: ProjectConfig): string {
