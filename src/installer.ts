...
import { getPackageManagerCommand, logSuccess, logWarning } from './utils.js';
...
export async function installDependencies(config: ProjectConfig): Promise<void> {
  ...
  try {
    ...
    logWarning(
      `Please run ${chalk.cyan(install)} in the ${chalk.cyan(config.projectName)} directory manually`
    );
    throw error;
  }
}

export async function runInitialBuild(config: ProjectConfig): Promise<boolean> {
